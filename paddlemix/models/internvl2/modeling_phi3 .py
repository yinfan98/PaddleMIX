...
PHI3_PRETRAINED_MODEL_ARCHIVE_LIST = ['microsoft/Phi-3-mini-4k-instruct',
...
class Phi3RMSNorm(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, eps=1e-06):
        ...
        """
        super().__init__()
        out_0 = paddle.create_parameter(shape=paddle.ones(shape=hidden_size
...
class Phi3RotaryEmbedding(paddle.nn.Layer):
    ...
    def forward(self, x, position_ids, seq_len=None):
        if self.inv_freq is None:
            self.inv_freq = 1.0 / self.base ** (paddle.arange(start=0, end=
                self.dim, step=2, dtype='int64').astype(dtype='float32') /
                self.dim)
        inv_freq_expanded = self.inv_freq[None, :, None].astype(dtype='float32'
            ).expand(shape=[tuple(position_ids.shape)[0], -1, 1])
        ...
        with paddle.amp.auto_cast(enable=False):
            x = inv_freq_expanded.astype(dtype='float32'
                ) @ position_ids_expanded.astype(dtype='float32')
...
class Phi3SuScaledRotaryEmbedding(Phi3RotaryEmbedding):
    ...
    def __init__(self, dim, config, device=None):
        super().__init__(dim, config.max_position_embeddings, config.
            rope_theta, device)
        self.short_factor = config.rope_scaling['short_factor']
        self.long_factor = config.rope_scaling['long_factor']
        self.original_max_position_embeddings = (config.
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        if seq_len > self.original_max_position_embeddings:
            ext_factors = paddle.to_tensor(data=self.long_factor, dtype=
                'float32', place=x.place)
        else:
            ext_factors = paddle.to_tensor(data=self.short_factor, dtype=
                'float32', place=x.place)
        inv_freq_shape = paddle.arange(start=0, end=self.dim, step=2, dtype
            ='int64').astype(dtype='float32') / self.dim
        ...
        with paddle.amp.auto_cast(enable=False):
            ...
            emb = paddle.concat(x=(freqs, freqs), axis=-1)
            scale = (self.max_position_embeddings / self.
                original_max_position_embeddings)
            if scale <= 1.0:
                scaling_factor = 1.0
            else:
                scaling_factor = math.sqrt(1 + math.log(scale) / math.log(
...
class Phi3MLP(paddle.nn.Layer):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.gate_up_proj = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=2 * config.intermediate_size, bias_attr=False)
        self.down_proj = paddle.nn.Linear(in_features=config.
            intermediate_size, out_features=config.hidden_size, bias_attr=False
            )
...
class Phi3Attention(paddle.nn.Layer):
    ...
    def __init__(self, config: Phi3Config, layer_idx: Optional[int]=None):
        ...
        op_size = self.num_heads * self.head_dim + 2 * (self.
        ...
        self.qkv_proj = paddle.nn.Linear(in_features=self.hidden_size,
            out_features=op_size, bias_attr=False)
        self._init_rope()

    def _init_rope(self):
        if self.rope_scaling is None:
            self.rotary_emb = Phi3RotaryEmbedding(self.head_dim,
                max_position_embeddings=self.max_position_embeddings, base=
                self.rope_theta)
        else:
            scaling_type = self.config.rope_scaling['type']
            if scaling_type == 'su':
                self.rotary_emb = Phi3SuScaledRotaryEmbedding(self.head_dim,
                    self.config)
            elif scaling_type == 'yarn':
                self.rotary_emb = Phi3YarnScaledRotaryEmbedding(self.
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        =None, past_key_value = None, output_attentions: bool=False, use_cache: bool=False) ->Tuple[
        ...
        key_states = qkv[..., query_pos:query_pos + self.
            num_key_value_heads * self.head_dim]
        value_states = qkv[..., query_pos + self.num_key_value_heads * self
            .head_dim:]
        ...
        x = value_states.view(bsz, q_len, self.num_key_value_heads, self.
            head_dim)
        ...
        perm_6[2] = 3
        perm_6[3] = 2
        attn_weights = paddle.matmul(x=query_states, y=x.transpose(perm=perm_6)
        ...
        attn_weights = paddle.nn.functional.softmax(x=attn_weights, axis=-1,
            dtype='float32').to(value_states.dtype)
...
class Phi3FlashAttention2(Phi3Attention):
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        Optional[paddle.Tensor]=None, position_ids: Optional[paddle.Tensor]
        =None, past_key_value =None, output_attentions: bool=False, use_cache: bool=False, **kwargs
        ...
        use_sliding_windows = _flash_supports_window_size and getattr(self.
            ...
            ) is not None and kv_seq_len > self.config.sliding_window
        if past_key_value is not None:
        ...
        attn_output = self._flash_attention_forward(query_states,
            ...
            attn_dropout, use_sliding_windows=use_sliding_windows)
        attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)
        attn_output = self.o_proj(attn_output)
        if not output_attentions:
            attn_weights = None
    ...
    def _upad_input(self, query_layer, key_layer, value_layer,
        ...
        indices_k, cu_seqlens_k, max_seqlen_in_batch_k = _get_unpad_data(
            attention_mask)
        key_layer = bert_padding.index_first_axis(key_layer.
            reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k)
        value_layer = bert_padding.index_first_axis(value_layer.
            reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k)
        if query_length == kv_seq_len:
            query_layer = bert_padding.index_first_axis(query_layer
            ...
            query_layer, indices_q, cu_seqlens_q, max_seqlen_in_batch_q = (
                bert_padding.unpad_input(query_layer,
                attention_mask))
...
class Phi3SdpaAttention(Phi3Attention):
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        Optional[paddle.Tensor]=None, position_ids: Optional[paddle.Tensor]
        =None, past_key_value=
        None, output_attentions: bool=False, use_cache: bool=False) ->Tuple[
        paddle.Tensor, Optional[paddle.Tensor], Optional[Tuple[paddle.Tensor]]
        ]:
...
PHI3_ATTENTION_CLASSES = {'eager': Phi3Attention, 'flash_attention_2':
    Phi3FlashAttention2, 'sdpa': Phi3SdpaAttention}
...
class Phi3DecoderLayer(paddle.nn.Layer):
    ...
    def __init__(self, config: Phi3Config, layer_idx: int):
        super().__init__()
        self.config = config
        self.self_attn = PHI3_ATTENTION_CLASSES[config.attn_implementation](
            config, layer_idx=layer_idx)
        self.mlp = Phi3MLP(config)
        self.input_layernorm = Phi3RMSNorm(config.hidden_size, eps=config.
            rms_norm_eps)
        ...
        self.post_attention_layernorm = Phi3RMSNorm(config.hidden_size, eps
            =config.rms_norm_eps)
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        False, **kwargs) ->Tuple[paddle.Tensor, Optional[Tuple[paddle.
...
class Phi3Model(Phi3PreTrainedModel):
    ...
    def __init__(self, config: Phi3Config):
        ...
        self.layers = paddle.nn.LayerList(sublayers=[Phi3DecoderLayer(
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.view(-1, seq_length).astype(dtype=
                'int64')
...
class Phi3ForCausalLM(Phi3PreTrainedModel):
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        ...
        Tuple, paddlenlp.transformers.model_outputs.CausalLMOutputWithPast]:
        """
        Args:
        ...
        outputs = self.model(input_ids=input_ids, attention_mask=
            ...
            past_key_values, inputs_embeds=inputs_embeds, use_cache=
...
class Phi3ForSequenceClassification(Phi3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.model = Phi3Model(config)
        self.score = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=self.num_labels, bias_attr=False)
        self.post_init()
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        ...
        paddle.Tensor]=None, use_cache: Optional[bool]=None,
        output_attentions: Optional[bool]=None, output_hidden_states:
        Optional[bool]=None, return_dict: Optional[bool]=None) ->Union[Tuple, SequenceClassifierOutputWithPast]:
        """
        ...
        if self.config.pad_token_id is None:
            ...
            sequence_lengths = paddle.equal(x=input_ids, y=self.config.
                pad_token_id).astype(dtype='int32').argmax(axis=-1) - 1
        ...
        if labels is not None:
            ...
            if self.config.problem_type is None:
                ...
                elif self.num_labels > 1 and (labels.dtype == 'int64' or 
                    labels.dtype == 'int32'):
        ...
        return SequenceClassifierOutputWithPast(
            loss=loss, logits=pooled_logits, past_key_values=model_outputs.
            past_key_values, hidden_states=model_outputs.hidden_states,
...
class Phi3ForTokenClassification(Phi3PreTrainedModel):
    ...
    def __init__(self, config: Phi3Config):
        ...
        elif hasattr(config, 'hidden_dropout'
    ...
    def forward(self, input_ids: Optional[paddle.Tensor]=None,
        ...
        ...]]=None, attention_mask: Optional[paddle.Tensor]=None,
        inputs_embeds: Optional[paddle.Tensor]=None, labels: Optional[
        paddle.Tensor]=None, use_cache: Optional[bool]=None,
        output_attentions: Optional[bool]=None, output_hidden_states:
        Optional[bool]=None, return_dict: Optional[bool]=None, **
        deprecated_arguments) ->Union[Tuple[paddle.Tensor], TokenClassifierOutput]:
        """
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[0, ...,
            config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss), If
        ...
        return_dict = (return_dict if return_dict is not None else self.
            config.use_return_dict)
        model_outputs = self.model(input_ids, past_key_values=
            past_key_values, attention_mask=attention_mask, inputs_embeds=
        ...
        if labels is not None:
            ...
            loss_fct = paddle.nn.CrossEntropyLoss()
            loss = loss_fct(logits.view(batch_size * seq_length, self.
                num_labels), labels.view(batch_size * seq_length))
        if not return_dict:
            output = (logits,) + model_outputs[2:]
            return (loss,) + output if loss is not None else output
        return paddle.transformers.model_outputs.TokenClassifierOutput(loss=
            loss, logits=logits, hidden_states=model_outputs.hidden_states,
            attentions=model_outputs.attentions)
